...
if ticker_data is not None and not ticker_data.empty:
    ...
    with col_results:
        ...
        try:
            ...
            col_graphs, col_performance = st.columns([3, 2])
...
if ticker_data is not None and not ticker_data.empty:
    ...
    with col2:
        ...
        # Placeholder for the selected strategy visualization
        if strategy_option == 'Buy and Hold':
            buy_and_hold_viz()
        elif strategy_option == 'SMA Cross':
            sma_cross_viz()
    ...
    with col2:
        st.subheader('Performance Metrics')
        key_metrics = ['Start', 'End', 'Duration', 'Exposure Time [%]', 'Equity Final [$]', 'Equity Peak [$]', 'Return [%]', 'Buy & Hold Return [%]', 'Return (Ann.) [%]', 'Volatility (Ann.) [%]', 'Sharpe Ratio', 'Sortino Ratio', 'Calmar Ratio', 'Max. Drawdown [%]', 'Avg. Drawdown [%]', 'Max. Drawdown Duration', 'Avg. Drawdown Duration', 'Trades', 'Win Rate [%]', 'Best Trade [%]', 'Worst Trade [%]', 'Avg. Trade [%]', 'Max. Trade Duration', 'Avg. Trade Duration', 'Profit Factor', 'Expectancy [%]']
        metrics = output.drop(['_strategy', '_equity_curve', '_trades'])
        selected_metrics = {k: metrics[k] for k in key_metrics if k in metrics}
        df_metrics = pd.DataFrame(selected_metrics, index=['Value']).T
        st.metric("Total Return", f"{df_metrics.loc['Return [%]', 'Value']:.2f}%")
    ...
    with col3:
        st.subheader('Trade Log')
        st.dataframe(output['_trades'], use_container_width=True, height=300)
        st.subheader('Comparison and Radar')
        fig_return_comparison = go.Figure(data=[
            go.Bar(name='Strategy', x=['Return'], y=[strategy_return]),
            go.Bar(name='Buy & Hold', x=['Return'], y=[bh_return])
        ])
